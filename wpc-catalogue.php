...
function catalogue() {
    ...
    /*echo  '<div class="wp-catalogue-breadcrumb"> <a href="'.$catalogue_page_url.'">' . __(get_field('all_products','option'), "wpc").'</a> &gt;&gt; <a href="'.$page_url.'">'.$page_name.'</a>  ' . $pname . '</div>';
    ...
    echo  '<ul class="wpc-categories">';
        ...
        // generating sidebar
        if($count>0){
            echo  '<li class="wpc-category ' . $class . '"><a href="'. get_option('catalogue_page_url') .'">'. __(get_field('all_products','option'), "wpc").'</a></li>';	
 ...
 // 
    ...
    if($term_slug){
        $terms  =   get_terms( array( 'slug' => $term_slug ) );
        echo '<h3 style="margin-left: 15px;"><i class="fa fa-arrow-right" aria-hidden="true"></i><a href="'.$catalogue_page_url.'">'.get_field('all_products','option').'</a></h3>';
    }
    
    $count  =   count($terms);

    //echo '<div class="col-sm-3"><img class="chile" src="'.get_bloginfo('template_directory').'/layout/images/fondo.jpg"> </div> 
    echo '<div class="col-sm-12 fruits">
                ';
            ...
            <h1>'.$term->name.'</h1>
                <div id="wpc-col-2 row">
                    ...
                    while($products->have_posts()): $products->the_post();
    					...
    					echo  '<!--wpc product-->';
                        echo  '<div class="wpc-product col-xs-12 col-sm-3">';
